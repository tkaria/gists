...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option("-o", "--outqueue", dest="outqueue",
                      help="SQS outbound queue name", default='outq')
    parser.add_option("-i", "--inqueue", dest="inqueue",
                      help="SQS inbound queue name", default='inq')
    parser.add_option("-d", "--debug", dest="debug",
                      help="Print debug information", default='False')
    parser.add_option("-t", "--terminate", dest="terminate",
                      help="Terminate instance when done", default=False)
    parser.add_option("-e", "--ephemeral", dest="ephemeral",
                      help="Use ephemeral storage on EC2", default=False)
    parser.add_option("-r", "--max_retries", dest="max_retries",
                      help="max number of retries before we accept queue is empty", default=3)
    parser.add_option("-w", "--retry_wait", dest="retry_wait",
                      help="time to sleep between retries", default=10)
    ...
    while retries < options.max_retries:
        ...
        if len(result_set) >= 1:
            ...
            bucket = m['bucket']
            s3_download_file.get_s3_file_to_local(
                s3cxn, bucket, input_file, input_file, STORAGE_PREFIX)
            ...
            # KTK - TODO - should wrap in try except block - to catch failed
            # upload
