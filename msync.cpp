/**
 * Build string: g++ -I /opt/src  msync.cpp  -o msync -lgtest -lpthread -lrt -lboost_date_time -ggdb
 */
...
TEST(WriteTest, TestMsync) {
  ...
  char* map = (char*)mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (map == MAP_FAILED) {
    close(fd);
...
TEST(WriteTest, TestIostream) {
  unsigned int seed = time(0);
  std::ofstream* f = new std::ofstream("iostream.txt",  std::ios::binary | std::ios::out);
  if (!f) {
