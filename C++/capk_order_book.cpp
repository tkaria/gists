...
*/
  ...
  std::ostream *pLog =
      new std::ofstream(logFileName, std::ios::out | std::ios::app);
...
*/
    ...
    FIX::UtcTimeStamp ts =
        FIX::UtcTimeStamp(FIX::UtcTimeStampConvertor::convert(vec[0]));
    ...
    capitalk::PriceDepthOrderBookEntry obe =
        capitalk::PriceDepthOrderBookEntry(ts, ua, ty, id, px, sz);
    b.enter(obe);
    try {
