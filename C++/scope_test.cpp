...
public:
  en(int x, std::string y) : _x(x), _y(y) {
    std::cerr << "ctor(" << x << "," << y << ")" << std::endl;
  };

  en(const en &e) : _x(e._x), _y(e._y) {
    std::cerr << "copy(" << e._x << "," << e._y << ")" << std::endl;
  };
